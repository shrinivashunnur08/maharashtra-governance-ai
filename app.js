...
=======
    try {
        ...
        console.error('Initialization error:', error);
        showError('Failed to initialize application. Please check your connection.');
    }
>>>>>>> 5b436b7282e3e63ee1fb96329e8b89953197f0f4
}
...
=======
    ...
    document.getElementById('download-geo')?.addEventListener('click', downloadGeoData);
>>>>>>> 5b436b7282e3e63ee1fb96329e8b89953197f0f4
}
...
function updateKPIs(requests) {
    ...
    const totalAffected = requests.reduce((sum, r) => sum + (r.affected_count || 0), 0);

    document.getElementById('total-requests').textContent = totalRequests;
    document.getElementById('open-requests').textContent = openRequests;
    document.getElementById('critical-requests').textContent = criticalRequests;
    document.getElementById('total-affected').textContent = totalAffected.toLocaleString();

    const openPercent = ((openRequests / totalRequests) * 100).toFixed(0);
    document.getElementById('open-trend').textContent = `${openPercent}% of total`;
}
...
function updateCriticalAlerts(requests) {
    ...
    if (criticalRequests.length === 0) {
        container.innerHTML = '<div class="alert-card"><p>No critical alerts at this time</p></div>';
        return;
    }

    container.innerHTML = criticalRequests.map(req => {
        const daysOpen = calculateDaysOpen(req.date_submitted);
        return `
>>>>>>> 5b436b7282e3e63ee1fb96329e8b89953197f0f4
...
=======
                    <strong>Description:</strong> ${req.description.substring(0, 150)}...<br>
>>>>>>> 5b436b7282e3e63ee1fb96329e8b89953197f0f4
...
<<<<<<< HEAD
    ...
    .join("");
=======
            <td><span class="severity-badge ${req.severity.toLowerCase()}">${req.severity}</span></td>
...
async function handleAnalyzeRequest() {
    ...
    try {
        // Call Supabase Edge Function for AI analysis
        const response = await fetch(`${supabaseClient.supabaseUrl}/functions/v1/analyze-complaint`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${supabaseClient.supabaseKey}`
        ...
        console.error('Analysis error:', error);
        // Show fallback prediction
        const fallbackPrediction = generateFallbackPrediction(window.currentRequest);
        displayPredictionResults(fallbackPrediction);
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<span>Generate AI Prediction with Gemini</span>';
...
function showTrackingError(requestId) {
  ...
  resultDiv.innerHTML = `
=======
            <p><strong>Submitted:</strong> ${new Date(request.date_submitted).toLocaleDateString()}</p>
            <p><strong>Severity:</strong> ${request.severity}</p>
            <p><strong>Description:</strong> ${request.description}</p>
            ${request.status === 'Resolved' ?
                `<p><strong>Resolved Date:</strong> ${new Date(request.resolved_date).toLocaleDateString()}</p>` :
                request.status === 'In Progress' ?
                '<p>Your complaint is being processed by the department</p>' :
                '<p>Your complaint is in queue. Expected action within 2-3 days.</p>'
            }
...
=======
    try {
        ...
        document.getElementById('resolved-count').textContent = `${resolved}/${requests.length}`;
        document.getElementById('resolution-time').textContent = `${avgDays} days`;
    } catch (error) {
        console.error('Error loading transparency data:', error);
    }
...
function calculateAverageResolutionTime(requests) {
    const resolved = requests.filter(r => r.status === 'Resolved' && r.resolved_date);
    if (resolved.length === 0) return 0;
...
function generateFallbackPrediction(request) {
    ...
    return {
        ...
        predicted_priority: request.severity,
        recommended_action: `Assign to ${request.department} immediately. Target resolution: 2-5 days.`,
        estimated_resolution_days: { 'Critical': 2, 'High': 5, 'Medium': 7, 'Low': 10 }[request.severity] || 7,
        resource_requirements: 'Deploy team with standard equipment and budget allocation',
        reasoning: `Based on ${request.severity} severity and ${request.affected_count} affected citizens`
    };
}
...
function generateFallbackForecast(requests) {
    return {
        forecast_date: new Date().toISOString().split('T')[0],
        demand_forecast: {
            water_supply: { predicted_requests: 15, change_percent: 15, confidence: 75, trend: 'Increasing' },
            healthcare: { predicted_requests: 11, change_percent: 10, confidence: 70, trend: 'Stable' },
            infrastructure: { predicted_requests: 16, change_percent: 12, confidence: 80, trend: 'Increasing' },
            electricity: { predicted_requests: 9, change_percent: 8, confidence: 72, trend: 'Stable' }
        },
        insights: 'Expecting 15-20% increase in service requests. Water and infrastructure departments need immediate resource reinforcement.'
    };
}
